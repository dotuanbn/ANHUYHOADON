...
import { Users, Package, FileText, Settings as SettingsIcon } from "lucide-react";
...
const QuickActions = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
      ...
      <Card className="bg-white shadow-lg hover:shadow-xl transition-shadow cursor-pointer" onClick={() => navigate('/settings')}>
        <CardHeader className="px-4 sm:px-6 py-4 sm:py-6">
          <CardTitle className="flex items-center text-orange-500 text-sm sm:text-base">
            <SettingsIcon className="w-4 h-4 sm:w-5 sm:h-5 mr-2" />
            Cài đặt hệ thống
          </CardTitle>
          <CardDescription className="text-xs sm:text-sm">
            Điều chỉnh toàn bộ thông tin và giá trị mặc định
          </CardDescription>
        </CardHeader>
      </Card>
    </div>
